...
- **Zoom limits follow the project unit**: minimum zoom is absolute (5 px/m, terrain overview). Maximum zoom: units of a centimeter and larger cap at **1:1 physical scale** (CSS anchor 1in ≡ 96px, ≈ 3779.5 px/m — the scale bar never displays cm/in/… larger than real size); units *smaller* than a centimeter may be magnified until **1 unit spans one physical centimeter** (mm → 10× life-size), all within the 400 px-per-unit ceiling. Switching units re-clamps the view, center-anchored. All physical caps are multiplied by the screen calibration.
