...
- **Alignment guides (inference)**: while placing a point — drawing without Shift, or dragging — a coordinate within ~8 screen pixels of an existing point's X or Y snaps to that point's coordinate, and a dotted amber guide line from the source point (marked with a ring) to the position shows *why* it snapped, SketchUp-style. Axes are independent: X can align to one point while Y aligns to another, the two guides meeting at their intersection. Point alignment beats the grid magnet on the axis it engages (the grid still magnets a non-aligned axis), and it works even with the grid hidden or snap off, since it references visible points rather than the grid. Alt/Option disables it along with all other snapping; Shift's direction/projection locks take precedence (no alignment while Shift is held). While dragging it follows the release-to-apply rule: guides and the dotted circle preview the landing position live, and release applies it.
