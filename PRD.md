...
- **Zoom limits are unit-independent**: the zoom-out floor is **content-based** — you cannot zoom out past the point where the terrain's bounding box (padded 24 px per side) fits the viewport, so the drawing always at least fills the view; the absolute 5 px/m floor applies only while nothing is drawn (or the outline has no extent). The ceiling is **10× physical 1:1 scale** (CSS anchor 1in ≡ 96px, multiplied by the screen calibration) in every unit. Switching units never changes the zoom. Past life-size the scale bar doesn't pretend otherwise — it shows an amber **magnification badge** (e.g. "× 2.5") whenever displayed lengths exceed their real on-screen size.
