...
- **Zoom limits follow the project unit**: minimum zoom is absolute (5 px/m, terrain overview); maximum zoom is the smaller of "1 active unit magnified to 400 px" and **1:1 physical scale** (using the CSS anchor 1in ≡ 96px, ≈ 3779.5 px/m) — so the scale bar can never display a length larger than its real size on screen. In practice: m caps at 400 px/m, cm at ~37.8 px/cm (one real centimeter), in at 96 px (one real inch), mm at ~3.78 px/mm.
...
| `PHYSICAL_PX_PER_M` | `src/components/Canvas.tsx` | 96/2.54 × 100 | 1:1 life-size zoom cap (CSS reference DPI; swap for a calibrated value if ever needed) |
